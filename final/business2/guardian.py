...
def crawl_url(url,k,array):
	...
	for link in (soup.find_all('a')) :
		...
		if s:
			...
			if ( not "rssfeeds" in s and not "newshome" in s and not "currentquote" in s and not "marketstats" in s and not "mutual_funds" in s and not "twitter" in s and "http://www.theguardian.com/business/" in s and not "google.co" in s ):
				if not s in array : 
					array.append(s)
					print>>f2,s +'\n'
	f2.close()	
