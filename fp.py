...
def writetofile():
	...
	try:
		...
		for link in soup.find_all('a'):
			...
			if(strin and not strin in dict):
				dict[strin]=1
				if(strin.endswith('.html') and (('http://www.firstpost.com/politics/' in strin))):
					urls.append(strin)
	except:
		print "error"
	...
	try:
		#with time_limit(300):
			...
			#page = urllib.urlopen(url)
			#soup = BeautifulSoup(page.read())
			storydiv=soup.find('div',{'class':'fullCont1'})
			story=""
	...
	#print "y"
		...
		#print comm
			...
			#print "c"
			if(len(comments)>0):
				f=open("/home/daivik/IR/fp/politics/Comments_"+str(cnt),"w")
				for comment in comments:
					try:
						...
						pass
				f.close()
				f=open("/home/daivik/IR/fp/politics/Article_"+str(cnt),"w")
...
if __name__=='__main__':
	...
	global browser
	browser = webdriver.Firefox()
	baseurl="http://www.firstpost.com/category/politics"
	#finished=[]
	urls.append(baseurl)
	writetofile()
	#maxcount=50
