...
class SeleniumSpider(CrawlSpider):
	...
	rules = (
		Rule(SgmlLinkExtractor(allow=('\.ece', )), callback='parse_page',follow=True),
	)
 
	def __init__(self):
		CrawlSpider.__init__(self)
		self.verificationErrors = []
		self.selenium = selenium("localhost", 4444, "*chrome", "http://www.thehindu.com/")
		self.selenium.start()
	...
	def parse_page(self, response):
		...
		time.sleep(2.5)
 		text=sel.page_source
 		soup=BeautifulSoup(text)
		#Do some crawling of javascript created content with Selenium
		#sel.get_text("//div")
		comments=coup.find_all('p',{'class','comment_text'})
		print response.url
		for cmt in comments:
			print cmt
		yield item
