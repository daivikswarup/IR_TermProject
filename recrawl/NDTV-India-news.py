...
def writetofile():
	baseurl="http://www.ndtv.com/india-news"
	#finished=[]
	urls.append(baseurl)
	global browser
	global cnt
	while len(urls)>0:
		...
		try:
			with time_limit(300):
				...
				print headline
				print 'c'
				date=soup.find('div',{'class','ins_dateline'}).contents[4].get('datetime')
				print date
				print 'd'
		...
		#print comments
				if(len(comments)>0):
					...
					# 		pass
					# f.close()
					f=open("/home/daivik/IR/recrawl/NDTV/politics/Article_"+str(cnt),"w")
					print 'i'
